...
**러닝타임 3:56 · 27컷 · 4테이크 · 데스크톱 1440 × 900 · 모바일 420 × 900 · 60fps**
...
**1. 발견은 한 번만 보여준다.** 꽃을 열어 그 안의 두 이야기를 보는 장면은 Reveal **뒤**에만 나옵니다. 앞에서 미리 보여주면 Reveal이 재방송이 됩니다.

**2. 한 테이크는 2분을 넘기지 않는다.** 목소리로 답하는 구간이 특히 그렇습니다. 상태는 `localStorage`에 남으므로 중간에 끊어도 이어집니다.

**3. 내레이션이 정적을 이기지 않는다.** Reveal의 첫 6.8초와 각 화면의 첫 2초에는 말을 얹지 않습니다. 아래 VO 중 `(선택)` 표시는 빼도 됩니다 — 다 넣으면 말이 너무 많습니다.

---

## 길이와 모바일

**길이** — 1–5분이면 2분대는 오히려 손해입니다. 심사자가 "이게 뭘 하는 건지"를 넘어 "이걸 실제로 쓰면 어떤 기분인지"까지 가려면 Reveal에 시간을 줘야 합니다. 3:56으로 잡되, 늘린 시간의 대부분은 **정적**과 **폰에서만 보이는 것**에 씁니다.

**모바일** — 별도의 막(테이크 C)으로 **본편에 들어갑니다.** 축소판이 아니라 구성이 실제로 다르게 접히기 때문입니다:

- 정원이 화면 폭에 억지로 맞춰지지 않고 **가운데로 당겨집니다** — 같은 그림을 가까이서. 꽃이 썸네일이 되지 않습니다.
- Reveal의 두 페이지가 **세로로 쌓이고**, 만남이 그 사이 틈에서 일어납니다. 좌우 구성을 눌러 담은 게 아니라 한 열을 위해 다시 짠 것입니다.
- 오늘의 질문은 정원을 덮지 않고 위로 비켜섭니다.

위치는 마지막 인상을 깎지 않도록 **Meet Her 다음, 엔드카드 직전**입니다.
...
| **상태** | 콘솔에서 `localStorage.clear()` 실행 후 새로고침. **테이크 A와 C 앞에서만** 하세요 — B1과 B2 사이에는 하면 안 됩니다. |
| **마이크** | 버리는 녹음을 한 번 먼저 해서 Chrome 권한 팝업을 미리 없애세요. 화면에 절대 나오면 안 됩니다. |
| **커서** | 커서 하이라이트 켜기, 클릭 물결 끄기. 천천히 움직이세요 — 앱의 속도가 요점이고, 커서가 튀면 그걸 망칩니다. |
| **리허설** | **15번 컷을 촬영 전에 한 번 끝까지** 돌려보세요. 두 이야기의 연결은 모델이 실시간으로 찾습니다. 꽃이 안 피면 스토리지 지우고 다시 하세요. |
...
| 0:30 | 0:22 | B1 | 7–9 | 몇 주 지난 정원 |
| 0:52 | 0:39 | B1 | 10–14 | 오늘의 질문 → 목소리로 답하기 → 둘 다 준비됨 |
| 1:31 | 0:25 | B1 | 15 | **Reveal (한 컷, 컷 없음)** |
| 1:56 | 0:16 | B1 | 16–17 | 좋아요가 아닌 질문, 그리고 다음 질문 |
| 2:12 | 0:26 | B2 | 18–19 | Between Us, 그리고 꽃 하나를 열어보기 |
| 2:38 | 0:36 | B2 | 20–21 | 아카이브, Meet Her |
| 3:14 | 0:32 | C | 22–26 | 한 손 안에서 |
| 3:46 | 0:10 | — | 27 | 엔드카드 |

**테이크 경계** — A 끝 `0:30`(스토리지 초기화), B1 끝 `2:12`(초기화 **금지**), B2 끝 `3:14`, C 끝 `3:46`.
...
> **VO** — Then & Now asks us the same question — one at a time.
...
> ⛔ **컷.** 녹화 정지 → `localStorage.clear()` → 새로고침 → 테이크 B1 준비.

---

# 테이크 B1 — 오늘의 질문과 Reveal · 0:30–2:12

**여기가 영상의 중심입니다.** 1분 42초를 한 번에 찍습니다. 대사를 틀려도 계속 돌리고 내레이션은 따로 다시 녹음하세요.
...
- **클릭하지 마세요.** 꽃을 여는 건 Reveal 뒤(19번 컷)의 몫입니다.
...
> **VO** — Today: what made you happy when you were twenty. She has already answered — and I can't see it until I have.
...
> **VO (선택)** — I answer in my own voice. The recording is the memory; the text is a convenience.
...
> **여기엔 VO를 얹지 마세요.** 말하면서 동시에 내레이션할 수 없습니다. 이 15초는 현장음만 씁니다 — 필요한 설명은 11번 컷에서 이미 했습니다.

> **한 호흡에 말하세요.** 브라우저가 잘못 알아들어도 앱은 녹음을 보관하고 위 문장으로 대체하니 테이크는 안전합니다. 다만 나중에 재생되는 건 실제로 말한 목소리입니다.

---

### 13 · 1:18 · 5초 · `/today` → `/garden`
...
### 14 · 1:23 · 8초 · `/garden` → `/reveal/c_everyday_joy`

**둘 다 준비됨**

- 정원이 멈추고 닫힌 봉오리가 그려진 카드 한 장이 가운데 섭니다.
- **5초 붙잡기.** 서두르지 마세요 — 다음 컷의 정적이 이 카드에서 시작됩니다.
- **Reveal together →** 클릭하고, 클릭 직후 손을 마우스에서 완전히 떼세요.

> **VO** — Neither of us sees the other's answer until both are in.

---

### 15 · 1:31 · 25초 · `/reveal/c_everyday_joy`
...
### 16 · 1:56 · 9초 · `/reveal/c_everyday_joy`
...
> ⛔ **컷. 스토리지를 지우지 마세요.** 방금 심은 이야기와 새 봉오리가 다음 테이크에 그대로 있어야 합니다.

---

# 테이크 B2 — 쌓인 것들 · 2:12–3:14

### 18 · 2:12 · 16초 · `/between-us`
...
### 19 · 2:28 · 10초 · `/between-us` → `/memory/c_the_call`

**꽃 하나를 열어보기**

- 위로 스크롤해 **The One You Call** 로 돌아와 **클릭**.
- 그 꽃이 자란 두 이야기가 좌우로 열리고, 꽃은 가운데에 크게 섭니다.
- 시선이 흐르게 두기: 1976 · Chicago, 복도 공중전화 / 2026 · Seattle, 늦은 밤 전화. 5초.
- 아래의 아직 안 핀 봉오리와 *"Ask Grandma →"* 에서 2초.

> **VO** — Each one keeps the two stories it grew out of, and the question it left open.

---

### 20 · 2:38 · 14초 · `/stories` → `/stories/discovered`

**서로에게 한 모든 이야기**

- 상단 **Stories** 클릭. 목록 위에서 붙잡기 — 1966부터 2026까지, 그녀의 손과 당신의 손이 같은 열에. 4초.
...
> ⛔ **컷.** 창을 420 × 900으로 줄이고 `localStorage.clear()` 후 테이크 C 준비.

---

# 테이크 C — 한 손 안에서 · 3:14–3:46

**창을 420 × 900 으로 줄이거나 실제 폰에서 녹화합니다.** 랜딩의 데모 링크로 들어가세요. 26번 컷까지 한 번에 찍습니다.

> **화면비 처리** — 이 막만 세로입니다. 좌우를 흐린 배경으로 채우지 마세요. **폰 프레임 목업 안에 넣거나**, 세로 영상 그대로 두고 양옆은 단색(`#f1f0ec`)으로 두는 편이 깨끗합니다.

### 22 · 3:14 · 5초 · 모바일 `/`

**같은 책, 세로로**

- 랜딩이 세로로 쌓입니다 — 뜯어낸 프린트가 위, 제목과 씨앗이 아래.
- 위에서 아래로 한 번 천천히 스크롤. 손글씨와 눌린 꽃이 그대로 살아 있는 것을 보여줍니다.

> **VO** — It is the same book on a phone.

---

### 23 · 3:19 · 6초 · 모바일 `/garden`

**정원이 가까이 당겨진다**

- 데모 링크로 정원 진입. **3초 손 떼기** — 줄기가 올라옵니다.
- 하단 탭 내비(Garden · Stories · Between Us)가 보이게 프레이밍.
- 꽃 이름 *Leaving Home* 이 읽히는지 확인하고 3초.

> **VO** — The garden doesn't shrink to fit. It moves in close — the same drawing, fewer plants, full size.

> **왜 중요한가** — 여기서 폭 전체를 욱여넣으면 꽃이 40픽셀짜리 썸네일이 됩니다. 좁은 화면에서는 가운데 절반만 보여주도록 되어 있습니다.

---

### 24 · 3:25 · 5초 · 모바일 `/today`

**답하기 시트**

- 카드의 **Answer** 탭. 오버레이가 화면을 꽉 채웁니다.
- 질문 → 파형 → 시계 → 둥근 버튼이 한 화면에 다 들어오는 것. **녹음은 하지 않습니다.**
- 우상단 **×** 로 닫기.

---

### 25 · 3:30 · 11초 · 모바일 `/reveal/c_the_call`

**세로로 접힌 Reveal** — 이 막의 핵심

- `/reveal/c_the_call` 로 이동. 이미 공개된 대화라 바로 꽃이 있습니다.
- **두 페이지가 세로로 쌓입니다.** 위는 그녀의 바랜 종이, 아래는 당신의 흰색.
- 위에서 아래로 **아주 천천히** 한 번 스크롤. 두 이야기 사이 틈에서 만남이 일어나는 것을 지나갑니다 — 봉오리, 그다음 반반인 꽃, 이름, 두 줄.
- 꽃과 *"Two generations apart. / The same phone call."* 위에서 4초.

> **VO** — And the meeting happens in the gap between them. Not the wide version squeezed — laid out again for one column.

---

### 26 · 3:41 · 5초 · 모바일 `/between-us`

**나머지도 접힌다**

- 하단 탭에서 **Between Us**. 조각 → 꽃 → 이름 순으로 쌓인 것을 한 번 스크롤. 3초.
- 탭에서 **Stories** 로 옮겨 목록이 폭에 맞게 접히는 것 2초.
...
- 데스크톱으로 돌아와 `/` 랜딩 스프레드 스틸, 또는 자란 정원을 붙잡은 프레임으로 컷.
...
당일 물릴 수 있는 건 다섯 가지고, 다섯 다 테이크를 살리는 길이 있습니다.

**Reveal에서 꽃이 안 핀다**
연결은 실시간으로 찾고, 모델은 **아무것도 못 찾을 권리**가 있습니다. 일부러 그렇게 만들었습니다 — 그래야 꽃이 피었을 때 그게 진짜라는 뜻이 됩니다. 카메라 앞에서 그러면 *"Two stories, kept side by side"* 가 대신 뜹니다. `localStorage` 지우고 테이크 B1을 다시 찍으세요. **촬영 전 리허설 필수.**
...
**B1과 B2 사이에 스토리지를 지웠다**
방금 심은 이야기와 새 봉오리가 사라져서 Between Us의 숫자가 영상 앞부분과 어긋납니다. 지웠다면 B1부터 다시 찍으세요.

**정원이 비어 보인다**
거의 항상 캡처 아티팩트입니다. 일부 녹화 도구가 SVG 레이어를 한 프레임 늦게 그립니다. 페이지 이동 후 마우스를 한 번 움직이세요. 계속되면 60fps 대신 30fps로 찍으세요.

**노트북 화면으로 찍고 있다**
폭 820px 아래에서는 정원이 가까이 당겨지며 식물을 덜 보여주고, 560px 아래에서는 페이지가 세로로 쌓입니다. 둘 다 정상 동작이고 테이크 C에서 일부러 보여주는 것이지만, A·B에는 넓은 구도가 필요합니다. 첫 테이크 전에 창이 정말 1440인지 확인하세요.
...
| 0:14 | 0:10 | 10, 12 | 오늘의 질문과 목소리로 답하기 |
| 0:24 | 0:22 | 15 | **Reveal, 무편집.** 앞의 정적만 다듬기 |
| 0:46 | 0:06 | 16 | "I never knew this" |
| 0:52 | 0:08 | 27 | 엔드카드 |
