...
**러닝타임 3:23 · 23컷 · 2테이크 · 1440 × 900 · 60fps**
...
**길이** — 1–5분이면 2분대는 오히려 손해입니다. 심사자가 "이게 뭘 하는 건지"를 넘어 "이걸 실제로 쓰면 어떤 기분인지"까지 가려면 Reveal에 시간을 줘야 하고, 그 앞뒤에 숨 쉴 자리가 필요합니다. 3:23으로 잡되 **길이를 늘린 만큼을 전부 정적에 씁니다** — 새 화면을 욱여넣는 데 쓰지 않습니다.

**모바일** — **본편에는 넣지 않습니다.** 제출 규정이 모바일을 요구하지 않고, 이 제품의 승부처(Reveal의 정적, 반반인 꽃)는 넓은 화면에서 가장 잘 읽힙니다.

시간이 아까워서가 아니라 **자리가 아깝기 때문**입니다. 영상의 마지막 15초는 인상이 굳는 자리인데, 거기서 이미 본 것들의 축소판을 보여주면 "Different worlds. Same age." 다음에 바로 엔드카드가 오는 것보다 약해집니다. 화면비가 바뀌는 것도 제작상의 이음매로 읽힙니다.

모바일은 **제출 글이나 README에 스크린샷 한 장**으로 넣으세요. 비용이 0이고, 궁금한 심사자는 배포 링크로 직접 엽니다. 그래도 영상에 넣어야 한다면 맨 뒤 부록에 15초 코다 사양을 남겨뒀습니다.
...
**노트북 화면으로 찍고 있다**
폭 820px 아래에서는 정원이 가까이 당겨지며 식물을 덜 보여주고, 560px 아래에서는 페이지가 세로로 쌓입니다. 둘 다 정상 동작이지만 본편에는 넓은 구도가 필요합니다. 첫 테이크 전에 창이 정말 1440인지 확인하세요.
...
넣지 않기를 권하지만, 규정이 바뀌거나 심사 기준에 반응형이 들어간다면 이 15초를 **22번 컷과 엔드카드 사이**가 아니라 **9번 컷 뒤**에 끼우세요. 마지막이 아니라 중간이면 인상을 깎지 않습니다.
...
- `/reveal/c_the_call` 로 이동 — 두 페이지가 **세로로 쌓이고**, 만남이 그 사이에서 일어납니다. 위아래로 한 번 천천히 스크롤. 6초.
...
> **편집 노트** — 이 컷만 화면 비율이 다릅니다. 좌우를 흐린 배경으로 채우지 말고 **폰 프레임 목업 안에 넣거나**, 세로 영상 그대로 두고 양옆은 단색(`#f1f0ec`)으로 두세요.

**영상 대신 제출 글에 넣는다면** — 스크린샷 두 장이면 충분합니다: `/garden`(가운데로 당겨진 정원 + 하단 탭)과 `/reveal/c_the_call`(세로로 쌓인 두 페이지, 그 사이의 꽃). 캡션 한 줄: *"Same garden, folded down to one page."*
