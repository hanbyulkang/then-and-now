...
**러닝타임 3:55 · 28컷 · 3테이크 · 데스크톱 1440 × 900 · 모바일 420 × 900 · 60fps**
...
**길이** — 1–5분이면 2분대는 오히려 손해입니다. 심사자가 "이게 뭘 하는 건지"를 넘어 "이걸 실제로 쓰면 어떤 기분인지"까지 가려면 Reveal에 시간을 줘야 하고, 그 앞뒤에 숨 쉴 자리가 필요합니다. 3:55로 잡되, 늘린 시간의 대부분은 **정적**과 **폰에서만 보이는 것**에 씁니다 — 같은 화면을 다시 보여주는 데 쓰지 않습니다.

**모바일** — 별도의 막(테이크 C)으로 **본편에 들어갑니다.** 축소판을 보여주려는 게 아닙니다. 폰에서는 구성이 실제로 **다르게 접힙니다**:

- 정원이 화면 폭에 억지로 맞춰지지 않고 **가운데로 당겨집니다** — 같은 그림을 가까이서 보는 것. 꽃이 썸네일이 되지 않습니다.
- Reveal의 두 페이지가 **세로로 쌓이고**, 만남이 그 사이 틈에서 일어납니다. 넓은 화면의 좌우 구성을 세로로 옮긴 게 아니라, 세로를 위해 다시 짠 것입니다.
- 오늘의 질문은 정원을 덮지 않고 위로 비켜섭니다.

이건 "반응형입니다"라는 주장이 아니라 **같은 생각을 다른 지면에 다시 앉힌 것**이고, 그건 보여줄 값어치가 있습니다. 대신 위치는 마지막 인상을 깎지 않도록 **Meet Her 다음, 엔드카드 직전**입니다.
...
| 3:13 | 0:32 | C | 23–27 | 한 손 안에서 — 같은 정원, 세로로 접힌 Reveal |
| 3:45 | 0:10 | — | 28 | 엔드카드 |
...
**창을 420 × 900 으로 줄이거나 실제 폰에서 녹화합니다.** `localStorage.clear()` 후 랜딩의 데모 링크로 다시 들어가세요. 여기부터 27번 컷까지 한 번에 찍습니다.
...
### 26 · 3:29 · 11초 · 모바일 `/reveal/c_the_call`

**세로로 접힌 Reveal** — 이 막의 핵심

- 이미 열어둔 `/reveal/c_the_call` 로 이동.
...
**노트북 화면으로 찍고 있다**
폭 820px 아래에서는 정원이 가까이 당겨지며 식물을 덜 보여주고, 560px 아래에서는 페이지가 세로로 쌓입니다. 둘 다 정상 동작이고 테이크 C에서 일부러 보여주는 것이지만, 테이크 A·B에는 넓은 구도가 필요합니다. 첫 테이크 전에 창이 정말 1440인지 확인하세요.
